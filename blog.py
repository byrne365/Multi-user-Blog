...
# check if user is existing or new login
...
# validates that user of the comment and post are not the same
...
# validates if user can modify the post or comment
...
# validates if the user can like the post
...
# validates the post or comment with the url safe key
    ...
    def likeunlikepost(self, url_safe_key, likeunlike):
        if self.validate_like(url_safe_key):
            if self.pc.like_post(likeunlike, self.user.key):
                ...
                return
        self.error("Oh dear somehting went wrong!")
...
# comment handler inherit from post handler
